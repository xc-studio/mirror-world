...
const imageName = ["a", "s", "d", "f", "g", "h", "j", "k", "l", "z", "x", "c", "v", "b", "n", "m", "q", "w", "e", "."];
...
window.onload = async function () {
    ...
    ctx.fillText(
        `${loadimage}個読み込み / ${mapMaxLength + Object.keys(ImageObj).length}個中`,
        canvas.width / 2,
        canvas.height / 2 + 40,
    );
...
function EventWaiter() {
    return new Promise((resolve) => document.addEventListener("keydown", resolve));
}
...
function 画面を描く() {
    ...
    for (let i = 0; i < 18; i++) {
        if (cameraPosition + i >= 0 && cameraPosition + i < 現在マップ.length) {
            ...
            for (let d = 0; d < 10; d++) {
                if (row[d] != "." && row[d] != "" && row[d] != null && row[d] != undefined) {
                    ctx.beginPath();
                    if (row[d] == "q") {
                        ctx.globalAlpha = Math.max(Math.abs(i * 60 - (camera % 60) - 480) / 640 - 0.3, 0);
                    } else {
                        ctx.globalAlpha = 1;
                    }
                    ctx.drawImage(image[row[d]], i * 60 - (camera % 60), 650 - (d * 60 + 85), 60, 60);
                }
            }
        }
    }
    ...
    ctx.globalAlpha = 1;
    if (向き >= 0) {
        ctx.drawImage(player1, playerX - camera + 60 * 8, 650 - playerY - 85, 60, 60);
    } else {
        ctx.drawImage(player2, playerX - camera + 60 * 8, 650 - playerY - 85, 60, 60);
    }
    ctx.fillStyle = "#000000";
...
async function collisionDetection() {
    if (gameMode == "3D") {
        ...
        if (
            当たり判定[現在マップ[Math.floor((playerX + 10) / 60)].split("")[Math.ceil((playerY - 10) / 60)]] ==
                "damage" ||
            当たり判定[現在マップ[Math.floor((playerX + 50) / 60)].split("")[Math.ceil((playerY - 10) / 60)]] ==
                "damage" ||
            当たり判定[現在マップ[Math.floor((playerX + 10) / 60)].split("")[Math.ceil((playerY - 50) / 60)]] ==
                "damage" ||
            当たり判定[現在マップ[Math.floor((playerX + 50) / 60)].split("")[Math.ceil((playerY - 50) / 60)]] ==
                "damage"
        ) {
            死亡();
        }
        if (
            当たり判定[現在マップ[Math.floor((playerX + 1) / 60)].split("")[Math.ceil((playerY - 1) / 60)]] == "goal" ||
            当たり判定[現在マップ[Math.floor((playerX + 59) / 60)].split("")[Math.ceil((playerY - 1) / 60)]] ==
                "goal" ||
            当たり判定[現在マップ[Math.floor((playerX + 1) / 60)].split("")[Math.ceil((playerY - 59) / 60)]] ==
                "goal" ||
            当たり判定[現在マップ[Math.floor((playerX + 59) / 60)].split("")[Math.ceil((playerY - 59) / 60)]] == "goal"
        ) {
            ゲームクリア処理();
        }
    } else {
        if (
            !(
                当たり判定[現在マップ[Math.floor(playerX / 60) + 1].split("")[Math.ceil((playerY - 0.1) / 60)]] ==
                    true ||
                当たり判定[現在マップ[Math.floor(playerX / 60) + 1].split("")[Math.ceil((playerY + 0.1 - 60) / 60)]] ==
                    true
            )
        ) {
            if (rightPress == true) {
        ...
        if (
            !(
                当たり判定[現在マップ[Math.floor((playerX - 1.2) / 60)].split("")[Math.ceil((playerY - 0.1) / 60)]] ==
                    true ||
                当たり判定[
                    現在マップ[Math.floor((playerX - 1.2) / 60)].split("")[Math.ceil((playerY + 0.1 - 60) / 60)]
                ] == true
            )
        ) {
        ...
        if (gameMode == "UFO") {
            ...
            if (gravity == 1) {
                if (
                    !(
                        当たり判定[
                            現在マップ[Math.floor((playerX + 0.5) / 60)].split("")[Math.floor((playerY - 0.1) / 60)]
                        ] == true ||
                        当たり判定[
                ...
                if (
                    !(
                        当たり判定[
                            現在マップ[Math.floor((playerX + 0.5) / 60)].split("")[Math.floor((playerY + 60.1) / 60)]
                        ] == true ||
                        当たり判定[
                            現在マップ[Math.floor((playerX + 60 - 0.5) / 60)].split("")[
        ...
        if (
            現在マップ[Math.floor((playerX + 1) / 60)].split("")[Math.ceil((playerY - 1) / 60)] == "w" ||
            現在マップ[Math.floor((playerX + 59) / 60)].split("")[Math.ceil((playerY - 1) / 60)] == "w" ||
            現在マップ[Math.floor((playerX + 1) / 60)].split("")[Math.ceil((playerY - 59) / 60)] == "w" ||
            現在マップ[Math.floor((playerX + 59) / 60)].split("")[Math.ceil((playerY - 59) / 60)] == "w"
        ) {
            gravity = 1;
        ...
        if (
            現在マップ[Math.floor((playerX + 1) / 60)].split("")[Math.ceil((playerY - 1) / 60)] == "e" ||
            現在マップ[Math.floor((playerX + 59) / 60)].split("")[Math.ceil((playerY - 1) / 60)] == "e" ||
            現在マップ[Math.floor((playerX + 1) / 60)].split("")[Math.ceil((playerY - 59) / 60)] == "e" ||
            現在マップ[Math.floor((playerX + 59) / 60)].split("")[Math.ceil((playerY - 59) / 60)] == "e"
        ) {
            gravity = -1;
            if (Math.abs(playerY変化量) > 10) {
                playerY変化量 = playerY変化量 * 0.5;
        ...
        if (
            playerX - camera <= -480 &&
            (当たり判定[現在マップ[Math.floor(playerX / 60) + 1].split("")[Math.ceil((playerY - 0.1) / 60)]] == true ||
                当たり判定[現在マップ[Math.floor(playerX / 60) + 1].split("")[Math.ceil((playerY + 0.1 - 60) / 60)]] ==
                    true)
        ) {
            死亡();
    ...
    if (level == mapMaxLength) {
        ...
        for (let i = 0; i < str.length; i++) {
            text.insertAdjacentHTML(
                "beforeend",
                `<span style="--delay: ${0 /*i / 5*/}s" class="rainbow-text pop-text">` + str.split("")[i] + "</span>",
            );
            await 待つ(100);
        }
